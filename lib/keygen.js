...
const AccumulatorPK = require('../wire').AccumulatorPublicKey
...
function encodeCertKeys (keys, buf, offset) {
  ...
  keys.acc.encode(buf, offset)
  offset += keys.acc.encode.bytes
...
function decodeCertKeys (buf, offset) {
  ...
  keys.acc = AccumulatorPK.decode(buf, offset)
  offset += AccumulatorPK.decode.bytes
...
function certKeysEncodingLength (keys) {
  let len = keys.acc.encodingLength()
