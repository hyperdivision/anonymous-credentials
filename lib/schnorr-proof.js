...
function prover (generators, randomOracle) {
  ...
  function genProof (secrets) {
    ...
    var t = typeof randomOracle === 'function'
      ? curve.scalarFrom(randomOracle(P_[0].toString(16)))
      : randomOracle
  ...
  function verify (P, proof, challenge) {
    const t = challenge || curve.scalarFrom(randomOracle(proof.P_[0].toString(16)))
...
function proverF12 (generators, randomOracle) {
  if (!randomOracle) randomOracle = oracle
  if (randomOracle instanceof Uint8Array) return proverF12(generators, () => randomOracle)

  function genProof (secrets) {
    const scalars = generators.map(rand)

    const products = generators.map((g, i) => F12.exp(g, scalars[i]))
    const P_ = products.reduce((acc, el) => F12.mul(acc, el))
  ...
  function verify (P, proof) {
    ...
    const products = generators.map((g, i) => F12.exp(g, proof.blinds[i]))
    var lhs = products.reduce((acc, el) => F12.mul(acc, el))

    var tP = F12.exp(P, t)
    var rhs = F12.mul(proof.P_, tP)
